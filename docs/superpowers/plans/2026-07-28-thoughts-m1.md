...
- `cargo test` must stay green and `cargo build` must emit **zero warnings**. Verified baseline on `main` at branch time: **36 tests passing, 0 warnings**.
- `npx svelte-check` must stay at **0 errors**. Verified baseline: 0 errors, 7 pre-existing warnings across 4 files — do not add to that count.
