...
class NotificationStorage(private val context: Context, private val jsonMapper: ObjectMapper) {
  ...
  fun writeActionGroup(actions: List<ActionType>) {
    // Klaxon patch (upstream bug in 2.3.3): the original loop used
    // `val i = type.id` as the SharedPreferences key suffix, so every
    // action overwrote the previous one's slot. getActionGroup reads
    // with numeric "id0"/"id1"/... keys though, so the read never
    // found anything — buttons rendered with empty titles. Switching
    // to forEachIndexed gives the matching numeric suffix.
    for (type in actions) {
      val editor = getStorage(ACTION_TYPES_ID + type.id).edit()
      editor.clear()
      editor.putInt("count", type.actions.size)
      type.actions.forEachIndexed { i, action ->
